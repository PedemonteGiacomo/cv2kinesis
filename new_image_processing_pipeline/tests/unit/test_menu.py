...
PROCESSORS = {
    "ThresholdCCL": "medical_image_processing.processing.threshold_ccl.ThresholdCCL",
    ...
    "LiverCCSimple": "medical_image_processing.processing.liver_cc_simple.LiverCCSimple",
}
...
def print_menu(processor=None, mode=None):
    ...
    print(
        "2. Scegli modalità (slice/folder)" + (f"   [Scelta: {mode}]" if mode else "")
    )
    print("3. Esegui test")
    print("4. Esci")
...
def run_processor(processor_name, img):
    ...
    if processor_name == "ThresholdCCL":
        ...
        proc = proc_class(
            window_center=-600, window_width=1500, min_blob_px=30000, closing_kernel=7
        )
        ...
        proc = proc_class(
            thr=110, median_k=9, close_k=7, min_area_px=20000, side="left"
        )
    else:
        proc = proc_class()
    result = proc.run(img)
        ...
        if choice == "1":
            ...
            try:
                ...
                if processor == "LiverParenchyma":
                    ...
                    folder = get_folder_path()
                    images = glob.glob(os.path.join(folder, "*.png")) + glob.glob(
